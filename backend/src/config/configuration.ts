export default () => {
  ...
  s3: {
    endpoint: process.env.S3_ENDPOINT ?? (isDev ? 'http://localhost:9000' : ''),
    ...
    publicBaseUrl: process.env.S3_PUBLIC_BASE_URL ?? process.env.S3_ENDPOINT ?? (isDev ? 'http://localhost:9000' : ''),
